...
frame_count = 0
print_interval = 10  # Print angle matrix every 10 frames

def update_pose(frame, sm, gait_controller, lines, angle_lines):
    global frame_count, angle_data
    dt = 0.05  # 50ms per frame
    frame_count += 1

    # Update velocities and moving state based on key states
    if key_states['up']:
    ...
    # Print angle matrix every 10 frames
    if frame_count % print_interval == 0:
        print(f"\nFrame {frame_count} Angle Matrix (degrees):")
        print("    Hip    Upper   Lower")
        print(f"RB: {angle_data[0,0]:6.2f} {angle_data[0,1]:6.2f} {angle_data[0,2]:6.2f}")
        print(f"RF: {angle_data[1,0]:6.2f} {angle_data[1,1]:6.2f} {angle_data[1,2]:6.2f}")
        print(f"LF: {angle_data[2,0]:6.2f} {angle_data[2,1]:6.2f} {angle_data[2,2]:6.2f}")
        print(f"LB: {angle_data[3,0]:6.2f} {angle_data[3,1]:6.2f} {angle_data[3,2]:6.2f}")

    # Update angle plot
    for i in range(12):
        angle_lines[i].set_data(range(frame_count), angle_data.flatten()[i::12])
...
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.set_title('SpotMicroStickFigure Bezier Gait (Use Arrow Keys to control)')
...
anim = animation.FuncAnimation(fig, update_pose, frames=None, fargs=(sm, gait_controller, lines, angle_lines),
