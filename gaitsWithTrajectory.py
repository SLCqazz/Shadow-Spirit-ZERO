...
class SimpleGaitController:
    def __init__(self, robot):
        ...
        self.servo_mappings = {
            ...
            'RB': {'hip': (950, 1), 'upper': (1130, -1), 'lower': (2000, -1)},
            'LF': {'hip': (1970, 1), 'upper': (1520, -1), 'lower': (990, -1)},
            'RF': {'hip': (2100, -1), 'upper': (1370, -1), 'lower': (1990, -1)}
        }
    ...
    def inverse_kinematics(self, x, y, z, leg_index, hip_angle):
        ...
        gamma_yz = -math.atan2(y, z)
        gamma_h_offset = -math.atan2(h, lyz)
        ...
        try:
            beta_angle = -math.acos(max(min(n / hl, 1.0), -1.0))
        except ValueError:
            beta_angle = 0.0
            print(f"Leg {leg_index}: Warning - Invalid value for beta, setting to 0")

        alfa_xzp = -math.atan2(x, lyz)
        try:
            alfa_off = math.acos(max(min((hu + n) / lxzp, 1.0), -1.0))
        except ValueError:
            alfa_off = 0.0
        ...
        hip_angle = 0 #max(min(gamma, self.max_angles['hip']), -self.max_angles['hip'])
    ...
    def update(self, dt):
        self.time += dt
        phases = {leg: ((self.time * self.freq) + self.phase_offsets[leg]) % 1 for leg in ['RB', 'RF', 'LF', 'LB']}
...
def main():
    ...
    try:
        while True:
            ...
            leg_angles, pwm_duty_cycles, foot_positions = gait_controller.update(dt)
