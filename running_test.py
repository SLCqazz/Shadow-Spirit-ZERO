...
def main():
    ...
    for step in range(int(simulation_duration / dt)):
        ...
        # Convert angles to PWM and send to hardware
        hardware.set_leg_angles(leg_angles)

        # Print angle matrix and PWM values
        print(f"\nTime: {elapsed_time:.2f}s")
        print("Leg Angles (degrees) and PWM values:")
        print("    Hip    Upper   Lower")
        legs = ['RB', 'RF', 'LF', 'LB']
        joints = ['hip', 'upper', 'lower']
        for i, leg in enumerate(legs):
            angles = [angle * r2d for angle in leg_angles[i]]
            pwm_values = [hardware.angle_to_pwm(leg, joint, angle) for joint, angle in zip(joints, angles)]
            print(f"{leg}: {angles[0]:6.2f} ({pwm_values[0]:4d}) {angles[1]:6.2f} ({pwm_values[1]:4d}) {angles[2]:6.2f} ({pwm_values[2]:4d})")
